#include "animaux-page.h"
#include "message-box.h"
#include "widgets/animal.h"

AnimauxPageWidget::AnimauxPageWidget(Database *db, QStackedLayout *layout, QWidget *parent)
    : QWidget(parent)
    , m_db(db)
    , m_stackedLayout(layout)
{
    ...
    QLabel *label = new QLabel(tr("Animaux"));
    ...
    columns->addItem("Identification");
    columns->addItem("Nom");
...
void AnimauxPageWidget::m_contextMenu()
{
    QMenu menu(this);
    ...
    QAction *addAct = new QAction(addIcon, tr("Créer un animal"), this);
    connect(addAct, SIGNAL(triggered()), this, SLOT(m_create()));
    menu.addAction(addAct);

    if (selection->hasSelection())
    {
        QAction *editAct = new QAction(editIcon, tr("Éditer l'animal"), this);
        connect(editAct, SIGNAL(triggered()), this, SLOT(m_edit()));
        menu.addAction(editAct);

        QAction *deleteAct = new QAction(deleteIcon, tr("Supprimer l'animal"), this);
...
void AnimauxPageWidget::m_create()
{
    if (m_stackedLayout == nullptr)
    {
        AnimalWidget *widget = new AnimalWidget(m_db, 0);
        QDialog *dlg = new QDialog(this);
        dlg->setLayout(widget->layout());
        dlg->setMinimumSize(800, 500);
        dlg->show();

        connect(widget, &AnimalWidget::canceled, this, [=] () {
            dlg->close();
        });

        connect(widget, &AnimalWidget::edited, this, [=] () {
            refresh();
            dlg->close();
        });
    }
...
void AnimauxPageWidget::m_edit()
{
    ...
    if (m_stackedLayout == nullptr)
    {
        AnimalWidget *widget = new AnimalWidget(m_db, id);
        QDialog *dlg = new QDialog(this);
        dlg->setLayout(widget->layout());
        dlg->setMinimumSize(800, 500);
        dlg->show();
...
void AnimauxPageWidget::m_delete()
{
    QSqlQuery query = QSqlQuery(m_db->getSqlDatabase());
    uint id = m_table->getSelectionModel()->selectedRows(0).first().data().toUInt();

    if (!query.exec(QString("SELECT * FROM Action WHERE idAnimal=%1").arg(QString::number(id))))
    {
    ...
    if (query.size() > 0)
    {
        QMessageBox::information(this, tr("GestionAsso :: Information"),
                                 tr("Il éxiste des actions avec cet animal.\n"
    ...
    if (!query.exec(QString("DELETE FROM Animal WHERE idAnimal=%1").arg(QString::number(id))))
    {
        MessageBox::showDatabaseError(m_db->getSqlDatabase().lastError().text(), this);
        return;
    }
...
void AnimauxPageWidget::refresh()
{
    QSqlQueryModel *sqlQueryModel = m_table->getSqlQueryModel();

    sqlQueryModel->setQuery("SELECT * FROM Animal", m_db->getSqlDatabase());
    ...
    sqlQueryModel->setHeaderData(2, Qt::Horizontal, QObject::tr("Identification"));
    sqlQueryModel->setHeaderData(3, Qt::Horizontal, QObject::tr("Nom"));
    sqlQueryModel->setHeaderData(4, Qt::Horizontal, QObject::tr("Description"));
    sqlQueryModel->setHeaderData(5, Qt::Horizontal, QObject::tr("Remarque"));
}

QItemSelectionModel *AnimauxPageWidget::getSelectionModel()
{
    return m_table->getSelectionModel();
}

void AnimauxPageWidget::exportation()
{
    QString textData;
    int rows = m_table->getFilterModel()->rowCount();
    int columns = m_table->getFilterModel()->columnCount();

    textData = "ID, Type, Identification, Nom, Description, Remarque\n";
